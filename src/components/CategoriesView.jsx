...
const CategoriesView = ({ 
  ...
  const handleAddCategory = () => {
    ...
    if (!isPro && categories.length >= 3) {
        alert("Limite de 3 categorias atingido no plano Grátis. Faça Upgrade para criar ilimitadas!");
        return;
    }
  ...
  const handleAddSubcategory = async (category) => {
    if (!isPro) {
        alert("Subcategorias são exclusivas do plano Pro.");
        return;
    }
  ...
  const handleAddPaymentMethod = () => {
    ...
    if (!isPro && paymentMethods.length >= 3) {
        alert("Limite de 3 formas de pagamento atingido no plano Grátis. Faça Upgrade!");
        return;
    }
    ...
    <div className="space-y-6 pb-24"> 
        <Card>
            <div className="flex justify-between items-start mb-4">
                <h3 className="font-bold text-lg text-teal dark:text-white flex items-center gap-2 font-poppins">
                    <Tag className="w-5 h-5 text-mint" /> Categorias
                </h3>
                {!isPro && (
                    <span className={`text-xs px-2 py-1 rounded-lg border ${categories.length >= 3 ? 'bg-red-100 text-red-600 border-red-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                        {categories.length}/3 Grátis
                    </span>
                )}
            </div>
            ...
            <div className="flex gap-3 mb-6 items-center">
                ...
                <button 
                    ...
                    className={`p-3 rounded-xl transition-colors shrink-0 shadow-sm active:scale-95 text-white ${!isPro && categories.length >= 3 ? 'bg-gray-300 cursor-not-allowed' : 'bg-mint hover:bg-[#00b57a]'}`}
                    title={!isPro && categories.length >= 3 ? "Limite atingido" : "Adicionar Categoria"}
                >
                    {!isPro && categories.length >= 3 ? <Lock size={20} /> : <Plus size={20}/>}
                </button>
            </div>
            ...
            <div className="space-y-4">
                {categories && categories.length > 0 ? (
                            ...
                            {isOpen && (
                                <div className="bg-gray-50 dark:bg-gray-900/50 p-4 border-t border-gray-100 dark:border-gray-700 animate-fadeIn">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-1 text-xs font-bold text-gray-400 uppercase tracking-wider"><Layers size={12} /> Subcategorias</div>
                                        {!isPro && <div className="text-[10px] bg-purple-100 text-purple-600 px-2 py-0.5 rounded font-bold flex items-center gap-1"><Lock size={10} /> Recurso Pro</div>}
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            type="text" 
                                            placeholder={isPro ? "Adicionar nova subcategoria..." : "Upgrade para adicionar subcategorias"} 
                                            disabled={!isPro}
                                            className="flex-1 text-xs px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:border-mint focus:ring-1 focus:ring-mint shadow-sm disabled:bg-gray-100 disabled:cursor-not-allowed" 
                                            value={newSubcat} 
                                        ...
                                        <button 
                                            ...
                                            disabled={!isPro}
                                            className={`px-4 py-1 border border-gray-200 dark:border-gray-600 rounded-lg text-xs font-bold transition-colors shadow-sm ${isPro ? 'bg-white dark:bg-gray-700 text-teal dark:text-white hover:bg-gray-50' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
                                        >
                                            + Add
                                        </button>
                                    </div>
                    ...
                    <p className="text-sm text-gray-400 text-center py-4 italic">Nenhuma categoria cadastrada.</p>
                )}
            </div>
        </Card>
        ...
        <Card>
            <div className="flex justify-between items-start mb-4">
                ...
                {!isPro && (
                    <span className={`text-xs px-2 py-1 rounded-lg border ${paymentMethods.length >= 3 ? 'bg-red-100 text-red-600 border-red-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                        {paymentMethods.length}/3 Grátis
                    </span>
                )}
            </div>
            ...
            <div className="flex gap-3 mb-6 items-center">
                ...
                <input 
                    className="flex-1 px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-teal dark:text-white text-sm outline-none focus:ring-2 focus:ring-mint transition-all min-w-0 disabled:bg-gray-100 disabled:cursor-not-allowed" 
                    placeholder="Ex: Dinheiro, Débito..." 
                    value={newPaymentMethod} 
                    onChange={(e) => setNewPaymentMethod(e.target.value)} 
                    onKeyPress={(e) => e.key === 'Enter' && handleAddPaymentMethod()} 
                    disabled={!isPro && paymentMethods.length >= 3}
                />
                <button 
                    onClick={handleAddPaymentMethod} 
                    disabled={!isPro && paymentMethods.length >= 3}
                    className={`p-3 rounded-xl transition-colors shrink-0 shadow-sm active:scale-95 text-white ${(!isPro && paymentMethods.length >= 3) ? 'bg-gray-300 cursor-not-allowed' : 'bg-mint hover:bg-[#00b57a]'}`}
                    title={(!isPro && paymentMethods.length >= 3) ? "Limite atingido" : "Adicionar"}
                >
                    {(!isPro && paymentMethods.length >= 3) ? <Lock size={20} /> : <Plus size={20}/>}
                </button>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:max-h-[400px] md:overflow-y-auto md:pr-1 custom-scrollbar">
