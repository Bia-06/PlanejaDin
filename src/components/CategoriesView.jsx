...
const CategoriesView = ({ 
  ...
  const handleUpdatePaymentColor = async (id, newColor) => {
      if (updatePaymentMethod) {
        ...
        console.warn("Função updatePaymentMethod não fornecida.");
      }
  };
    ...
    <div className="space-y-6 pb-24"> 
        ...
        <Card>
            ...
            <div className="flex gap-3 mb-6 items-center">
                ...
                <button onClick={handleAddCategory} className="bg-mint text-white p-3 rounded-xl hover:bg-[#00b57a] transition-colors shrink-0 shadow-sm"><Plus size={20}/></button>
            </div>
                    ...
                    <div key={cat.id} className="border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-800 transition-all shadow-sm">
                        ...
                        <div className="flex flex-col p-4 gap-3">
                            <div className="flex justify-between items-center gap-2">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className="flex items-center gap-2 flex-1 overflow-hidden">
                                            ...
                                            <button 
                                                ...
                                                className="text-gray-400 hover:text-teal dark:hover:text-mint transition-colors shrink-0"
                                                title="Editar nome"
                                            >
                                                <Pencil size={14} />
                                            </button>
                                ...
                                <div className="flex items-center gap-2 md:gap-3">
                                    <span className="hidden md:block text-sm font-mono font-bold text-gray-600 dark:text-white">
                                        {formatCurrency(totalVal)}
                                    </span>
                                    
                                    <button 
                                        onClick={() => setActiveCategoryId(isOpen ? null : cat.id)}
                                        className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                                            isOpen 
                                                ? 'bg-mint text-white shadow-md' 
                                                : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600 dark:hover:text-mint'
                                        }`}
                                    >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="md:hidden text-xs font-mono font-bold text-gray-600 dark:text-white whitespace-nowrap">
                                    {formatCurrency(totalVal)}
                                </span>
                            </div>
                        </div>
                        ...
                        {isOpen && (
                            <div className="bg-gray-50 dark:bg-gray-900/50 p-4 border-t border-gray-100 dark:border-gray-700 animate-fadeIn">
                                <div className="flex items-center gap-1 text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">
                                    <Layers size={12} /> Subcategorias
                                </div>
                                <div className="space-y-2 mb-4">
                                    {cat.subcategories && cat.subcategories.map((sub, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className="flex items-center justify-between bg-white dark:bg-gray-800 p-2.5 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                                                <div className="flex items-center gap-3 flex-1 overflow-hidden">
                                                    ...
                                                    {isEditingSub ? (
                                                        <div className="flex items-center gap-2 flex-1">
                                                            <input 
                                                                autoFocus
                                                                type="text" 
                                                                value={editingSub.newName}
                                                                onChange={(e) => setEditingSub({...editingSub, newName: e.target.value})}
                                                                onKeyDown={(e) => e.key === 'Enter' && handleSaveSubcategoryName(cat)}
                                                                className="flex-1 text-xs font-medium text-gray-700 dark:text-gray-300 border-b border-mint outline-none bg-transparent min-w-0"
                                                            />
                                                        ...
                                                        <div className="flex items-center gap-2 flex-1 overflow-hidden">
                                                            ...
                                                            <button 
                                                                onClick={() => setEditingSub({ catId: cat.id, oldName: sub, newName: sub })}
                                                                className="text-gray-400 hover:text-teal dark:hover:text-mint transition-colors shrink-0"
                                                            >
                                                                <Pencil size={12} />
                                                            </button>
                                    ...
                                    {(!cat.subcategories || cat.subcategories.length === 0) && (
                                        <div className="text-center py-2 bg-white dark:bg-gray-800 rounded-lg border border-dashed border-gray-200 dark:border-gray-700">
                                ...
                                <div className="flex gap-2">
                                    <input 
                                        ...
                                        className="flex-1 text-xs px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:border-mint focus:ring-1 focus:ring-mint shadow-sm"
                                        value={newSubcat}
        ...
        <Card>
            ...
            <div className="flex gap-3 mb-6 items-center">
                ...
                <input 
                    ...
                    placeholder="Ex: Dinheiro, Débito, Pix..." 
                    value={newPaymentMethod} 
                    onChange={(e) => setNewPaymentMethod(e.target.value)} 
                    onKeyPress={(e) => e.key === 'Enter' && handleAddPaymentMethod()} 
                />
                <button onClick={handleAddPaymentMethod} className="bg-mint text-white p-3 rounded-xl hover:bg-[#00b57a] transition-colors shrink-0 shadow-sm"><Plus size={20}/></button>
            </div>
                            ...
                            <div key={method.id} className="flex justify-between items-center p-3 border border-gray-100 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 shadow-sm relative overflow-hidden group">
                                ...
                                <div className="flex items-center gap-3 flex-1 overflow-hidden">
                                    ...
                                    <div className="relative w-6 h-6 rounded-full border-2 border-gray-200 dark:border-gray-600 overflow-hidden shadow-sm shrink-0 cursor-pointer hover:border-mint dark:hover:border-mint transition-colors" style={{ backgroundColor: method.color || '#ccc' }}>
                                        <input
                                            type="color"
                                            value={method.color || '#F472B6'}
                                            onChange={(e) => handleUpdatePaymentColor(method.id, e.target.value)}
                                            className="absolute -top-1/2 -left-1/2 w-[200%] h-[200%] cursor-pointer p-0 border-0 opacity-0"
                                            title="Mudar cor do pagamento"
                                        />
                                    </div>
                                    ...
                                    {isEditingPayment ? (
                                        <div className="flex items-center gap-2 flex-1">
                                            <input 
                                                ...
                                                onKeyDown={(e) => e.key === 'Enter' && handleSavePaymentName()}
                                                className="flex-1 text-sm font-bold text-gray-700 dark:text-gray-200 border-b-2 border-mint outline-none bg-transparent px-1 py-0.5 min-w-0"
                                            />
                                        ...
                                        <div className="flex items-center gap-2 flex-1 overflow-hidden">
                                            ...
                                            <button 
                                                onClick={() => setEditingPayment({ id: method.id, name: method.name })}
                                                className="text-gray-400 hover:text-teal dark:hover:text-mint transition-colors shrink-0"
                                                title="Editar nome"
                                            >
                                                <Pencil size={14} />
